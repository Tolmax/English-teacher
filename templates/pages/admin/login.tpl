  ...
  <main class="login-page">
    <section class="card stack login-card" aria-labelledby="login-title">
      <div class="section__header">
        ...
        <p class="section__lead">Введите email и пароль, чтобы управлять классами и ответами учеников.</p>
      </div>
      ...
      <form class="form" action="<?= HOST ?>admin/login" method="post" novalidate>
        <div class="form__field">
          ...
          <input class="input" type="email" id="email" name="email" value="<?= e($old['email'] ?? '') ?>" autocomplete="email" required aria-describedby="email-error" aria-invalid="<?= !empty($errors['email']) ? 'true' : 'false' ?>">
