...
- `status` — `draft` или `ready`; старое значение `published` читается как `ready` для совместимости.
...
Также создаются рабочие страницы классов без демонстрационных материалов, тестов, ответов учеников и вопросов.
