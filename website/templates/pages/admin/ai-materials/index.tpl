...
$testForm = $testForm ?? [];
$selectedProvider = $testForm['ai_provider'] ?? $selectedProvider;
$defaultTestTemplateKey = $testForm['test_template'] ?? $defaultTestTemplateKey;
          ...
          <section class="card" aria-labelledby="ai-test-generator-title">
            ...
            <form class="form" action="<?= HOST ?>admin/ai-materials/generate-test" method="post" novalidate>
              ...
              <div class="form__field">
                ...
                <select class="select" id="class_title" name="class_title" required>
                  ...
                  <?php foreach ($classes as $class): ?>
                    <option value="<?= e($class['title']) ?>"<?= (string)($testForm['class_title'] ?? '') === (string)$class['title'] ? ' selected' : '' ?>><?= e($class['title']) ?> класс</option>
                  <?php endforeach; ?>
                </select>
              </div>
              ...
              <div class="form__field">
                ...
                <input class="input" type="text" id="topic" name="topic" value="<?= e($testForm['topic'] ?? '') ?>" placeholder="Например: Названия 10 зверей" required>
              </div>

              <div class="form__field">
                <label class="form__label" for="knowledge_source_id">Учебник / источник базы знаний</label>
                <select class="select" id="knowledge_source_id" name="knowledge_source_id" aria-describedby="knowledge-source-hint">
                  <option value="0">Без учебника — тест по теме</option>
                  <?php foreach (($knowledgeSources ?? []) as $source): ?>
                    <option value="<?= (int)$source['id'] ?>"<?= (int)($testForm['knowledge_source_id'] ?? 0) === (int)$source['id'] ? ' selected' : '' ?>><?= e($source['title']) ?></option>
                  <?php endforeach; ?>
                </select>
                <span class="form__hint" id="knowledge-source-hint">Добавьте активный источник в <a href="<?= HOST ?>admin/ai-knowledge">базу знаний</a> и заполните «Текст источника». Один загруженный PDF или скан без текста недостаточен.</span>
              </div>
              <div class="form__field">
                <label class="form__label" for="lesson_reference">Номер или название урока</label>
                <input class="input" type="text" id="lesson_reference" name="lesson_reference" maxlength="160" value="<?= e($testForm['lesson_reference'] ?? '') ?>" placeholder="Например: Unit 3 или School life" aria-describedby="lesson-reference-hint">
                <span class="form__hint" id="lesson-reference-hint">Заполните при выборе учебника. Ищем точный заголовок урока в тексте; номер 3 подходит для Lesson 3 / Unit 3. Если совпадений несколько, уточните название. <a href="<?= HOST ?>admin/teacher-guide#guide-textbook">Как подготовить учебник</a>.</span>
              </div>

              <div class="form__field">
                <label class="form__label" for="source_notes">Что учесть</label>
                <textarea class="textarea" id="source_notes" name="source_notes" rows="4" placeholder="Напишите слова, грамматику, формат или ограничения. Например: только Present Simple, короткие вопросы, без сложных слов."><?= e($testForm['source_notes'] ?? '') ?></textarea>
              </div>
              ...
              <fieldset class="form__field">
                ...
                <div class="admin-ai-options">
                  <label class="admin-ai-option">
                    <input type="radio" name="generation_mode" value="ai"<?= ($testForm['generation_mode'] ?? 'ai') !== 'manual' ? ' checked' : '' ?>>
                    <span>
                  ...
                  <label class="admin-ai-option">
                    <input type="radio" name="generation_mode" value="manual"<?= ($testForm['generation_mode'] ?? 'ai') === 'manual' ? ' checked' : '' ?>>
                    <span>
