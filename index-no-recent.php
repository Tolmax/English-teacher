...
if (($_GET['action'] ?? '') === 'open-output-folder') {
    openLocalFolder(APP_DATA_ROOT . 'storage' . DIRECTORY_SEPARATOR . 'output');
    header('Location: /');
    exit;
}

if (($_GET['action'] ?? '') === 'download-presentation') {
    $presentationPath = generatedPresentationPath((string)($_GET['run'] ?? ''), (string)($_GET['file'] ?? ''));
    if ($presentationPath === '') {
        http_response_code(404);
        exit('Презентация не найдена.');
    }

    header('Content-Type: application/vnd.openxmlformats-officedocument.presentationml.presentation');
    header('Content-Disposition: attachment; filename="' . rawurlencode(basename($presentationPath)) . '"');
    header('Content-Length: ' . filesize($presentationPath));
    readfile($presentationPath);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $runDir = APP_DATA_ROOT . 'storage' . DIRECTORY_SEPARATOR . 'output' . DIRECTORY_SEPARATOR . $runId . DIRECTORY_SEPARATOR;
        $imageDir = APP_DATA_ROOT . 'storage' . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . $runId . DIRECTORY_SEPARATOR;
