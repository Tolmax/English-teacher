...
function generateAiInteractiveTestDraft(array $material, array $knowledgeContext = [], ?string $provider = null): array
{
    ...
    if ($provider === 'gigachat') {
        ...
        return requestGigaChatAiInteractiveTestDraft($material, $knowledgeContext);
    }
...
function requestOpenAiInteractiveTestDraft(array $material, array $knowledgeContext = []): array
{
    ...
    if ((string)($material['test_template'] ?? '') === 'reading_true_false') {
...
function requestGigaChatAiInteractiveTestDraft(array $material, array $knowledgeContext = []): array
{
    $lastException = null;
    ...
    throw new RuntimeException(
        'GigaChat вернул тест не в нужном формате. Тест не создан, чтобы не подставить неправильную заготовку. ' .
