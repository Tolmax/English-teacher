...
require_once ROOT . 'app/services/gigachat-service.php';
...
function generateAiTeachingMaterial(array $material, array $knowledgeContext = [], ?string $provider = null): array
{
    ...
    if ($provider === 'gigachat') {
        ...
        return requestGigaChatTeachingMaterial($material, $knowledgeContext);
    }
...
function getAiTextProvider(?string $provider = null): string
{
    ...
    return $provider === 'gigachat' ? 'gigachat' : 'openai';
}
...
function getAiImageProvider(?string $provider = null): string
{
    ...
    if (in_array($provider, ['openai', 'gigachat', 'mock'], true)) {
        return $provider;
    }
...
function getAiImageProviderOptions(): array
{
    return [
        ...
        'gigachat' => [
            'label' => 'GigaChat',
            'model' => function_exists('getGigaChatImageModelName') ? getGigaChatImageModelName() : getGigaChatModelName(),
            'configured' => isGigaChatConfigured(),
        ],
        'mock' => [
            'label' => 'Mock',
...
function generateMockAiTeachingMaterial(array $material, array $knowledgeContext = []): array
{
    ...
    $content = implode("\n\n", [
        ...
        'Это mock-материал для локальной проверки без OPENAI_API_KEY/GIGACHAT_AUTH_KEY.',
    ]);
...
function requestGigaChatTeachingMaterial(array $material, array $knowledgeContext = []): array
{
    $result = requestGigaChatText(
