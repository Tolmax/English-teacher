...
function buildWordPresentationPptx(array $presentation, array $cards, string $outputPath): void
{
    if (!class_exists('ZipArchive')) {
        throw new RuntimeException('РќР° СЃРµСЂРІРµСЂРµ РЅРµРґРѕСЃС‚СѓРїРЅРѕ СЂР°СЃС€РёСЂРµРЅРёРµ ZipArchive РґР»СЏ СЃР±РѕСЂРєРё PPTX.');
    }

    $cards = array_values($cards);
    if (empty($cards)) {
        throw new RuntimeException('РќРµС‚ РєР°СЂС‚РѕС‡РµРє РґР»СЏ СЃР±РѕСЂРєРё РїСЂРµР·РµРЅС‚Р°С†РёРё.');
    }

    $dir = dirname($outputPath);
    if (!is_dir($dir) && !mkdir($dir, 0755, true)) {
        throw new RuntimeException('РќРµ СѓРґР°Р»РѕСЃСЊ СЃРѕР·РґР°С‚СЊ РїР°РїРєСѓ РґР»СЏ РїСЂРµР·РµРЅС‚Р°С†РёРё.');
    }

    $zip = new ZipArchive();
    if ($zip->open($outputPath, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== true) {
        throw new RuntimeException('РќРµ СѓРґР°Р»РѕСЃСЊ СЃРѕР·РґР°С‚СЊ PPTX-С„Р°Р№Р».');
    }
    ...
    if (!$zip->close()) {
        throw new RuntimeException('РќРµ СѓРґР°Р»РѕСЃСЊ СЃРѕС…СЂР°РЅРёС‚СЊ PPTX-С„Р°Р№Р».');
    }
}
...
function pptxSlideXml(array $slide, int $slideNumber, string $imageRelId = ''): string
{
    ...
    if ($isCover && $imageRelId !== '') {
        $imageShape = pptxPictureXml(3, 'Presentation cover image', $imageRelId, 609600, 685800, 5030000, 5480000);
    } elseif ($isCover) {
        $imageShape = pptxCoverIllustrationXml();
    } elseif ($imageRelId !== '') {
        $imageShape = pptxPictureXml(3, 'Word image', $imageRelId, 609600, 685800, 5030000, 5480000);
    } else {
        $imageShape = pptxShapeXml(3, 'Image placeholder', 609600, 685800, 5030000, 5480000, 'E9EFF8', 'BFD0E8', $imageText, 2000, '6B7890');
    }
    ...
    return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        ...
        . $imageShape
        . pptxShapeXml(4, 'Word', 5940000, 1150000, 5480000, 1040000, 'FFFFFF', 'FFFFFF', $title, $isCover ? 3600 : 5200, '1D4F91', true)
        . pptxShapeXml(5, 'Transcription', 5940000, 2440000, 5480000, 700000, 'FFFFFF', 'FFFFFF', $transcription, 2400, '536177')
        . pptxShapeXml(6, 'Hint', 5940000, 3470000, 5480000, 1700000, 'FFFFFF', 'D8E2F1', $hint, 2200, '1E2633')
